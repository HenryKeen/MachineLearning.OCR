...
let path = @"C:\Projects\Workshops\Digits\Digits\trainingsample.csv"
...
let rowsAndColumns = File.ReadAllLines(path) |> Array.map (fun s -> s.Split(','))
...
let rowsAndColumns = rows.[1..] |> Array.map (fun s -> s.Split(','))
...
let parseRow (row:array<string>) =
    row |> Array.map (fun s -> s.Split(',') |> Array.map(fun s2 -> int s2))

let rowsAndColumns = rows.[1..] |> parseRow
...
let rowsAndColumns = rows.[1..] |> parseRowIntoInts |> Array.map(fun r -> { Label = r.[0]; Pixels = r.[1..] } )
...
let getDistanceFromPoints p1 p2 = 
    (p1 - p2) * (p1 - p2)
    
let getDistanceForArray (a1:int[]) (a2:int[]) =
    Array.map2(getDistanceFromPoints) a1 a2 
    |> Array.sum 
    |> Convert.ToDouble 
    |> Math.Sqrt
...
let loadNumberSamples path =
    let rows = (File.ReadAllLines path).[1..]
    rows.[1..] 
    |> parseRowIntoInts 
